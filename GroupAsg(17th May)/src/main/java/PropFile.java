...
public class PropFile implements Runnable {
    ...
    static File propFile = new File("C:\\Users\\Tyjia\\IdeaProjects\\GroupAsg\\top3.properties"); //change the location
        ...
        try {
            ...
            while ((url = br.readLine()) != null) {
                ...
                for (Element row : doc.select("table.CRs1 tr")) {
                    if(row.select("td:nth-child(1)").text().equals("1")||row.select("td:nth-child(1)").text().equals("2")||row.select("td:nth-child(1)").text().equals("3")) {
